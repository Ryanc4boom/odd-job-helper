  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (mode === "signup") {
        ...
        toast.success("Welcome to Odd Job! Check your email to confirm.");
        navigate("/feed");
      } else {
      ...
      <div className="container flex items-center justify-center py-12 md:py-20">
        <Card className="w-full max-w-md rounded-3xl border-border/60 p-8 shadow-soft">
          <div className="mb-6 flex flex-col items-center text-center">
            ...
            <h1 className="mt-4 text-2xl font-extrabold">{mode === "signin" ? "Welcome back" : "Join Odd Job"}</h1>
            <p className="mt-1 text-sm text-muted-foreground">{mode === "signin" ? "Sign in to your account" : "Create an account in seconds"}</p>
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            {mode === "signup" && (
              <div className="space-y-2">
                ...
                <Input id="name" value={name} onChange={(e) => setName(e.target.value)} placeholder="Alex Neighbor" maxLength={80} className="rounded-xl" />
              </div>
            )}
            <div className="space-y-2">
